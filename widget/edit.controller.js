...
(function () {
  ...
  editPlaybookButtons110Ctrl.$inject = [
    "$scope",
    "$uibModalInstance",
    "config",
    "FormEntityService",
  ...
  function editPlaybookButtons110Ctrl(
    $scope,
    $uibModalInstance,
    config,
    FormEntityService,
    ...
    function playbookButton() {
      $scope.playbookList = angular.copy(
        $scope.config.selectedPlaybooksWithRecord
      );
    }
    ...
    function updatePlaybookList(playbook) {
      angular.forEach(
        $scope.modulePlaybooks,
        function (playbookDetails, index) {
    ...
    function removeButtonWithRecord(index, action) {
      ...
      $scope.config.selectedExecutionWizardPlaybooks = _.reject(
        $scope.config.selectedExecutionWizardPlaybooks,
    ...
    function _init() {
      ...
      playbookService
        .getActionPlaybooks($scope.entity, true)
        .then(function (playbooks) {
          $scope.modulePlaybooks = playbooks;
          playbookButtonList = angular.copy(
            $scope.config.selectedPlaybooksWithRecord
          );
          angular.forEach(
            $scope.config.selectedPlaybooksWithRecord,
